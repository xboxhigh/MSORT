int ms_fillMem( char *ptrBuf, const uint uiBSize, FILE *fp )
{
	uint	uiLen, iLines, iEOF;
	char	*pszLine, *ptrEnd;
	char	**ptraLines;
	char	*pszCurrent, iRead;
	
	ptrEnd=ptrBuf+uiBSize;
	ptraLines=(char **)(ptrEnd);
	iRead=iLines=0;
	*pszCurrent=ptrBuf;
	ptrDataEnd=ptrBuf+iRead;
	iEOF=0;
		
	while( (iRemain=((char *)ptraLines - ptrCurrent)) >= iReadSize )
	{
		*ptraLines=pszCurrent;
		while( *pszCurrent != '\n' )
		{
			if( pszCurrent >= ptrDataEnd )
			{
				if( ((char *)ptraLines - ptrCurrent)) < iReadSize +size(char *) || iEOF)
				{
					qsort( ptrBuf, iLines, sizeof( char *), ms_strcmp );
					ms_fbwrite();
					if( iEOF )
						return 1;
					iRemain=pszCurrent-*ptraLines;
					memcpy( ptrBuf, *ptraLines, iRemain );
					pszCurrent=ptrBuf+iRemain;
					ptraLines=(char **)(ptrEnd);
					*ptrLines=ptrBuf;
					iLines=0;
					iRead=fread( ptrBuf+iRemain, sizeof(char), iReadSize-iRemain, *fp);
				}else
					iRead=fread( pszCurrent, sizeof(char), iReadSize, *fp);
				ptrDataEnd=pszCurrent+iRead;
				if( feof(fp) )				
					iEOF=1;
			}
			else
	...
	if( iLines > 0)
	{
		qsort( ptraLines, iLines, sizeof( char *), ms_strcmp );
		ms_fbwrite();
	};
}
